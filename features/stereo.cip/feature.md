...
The implemented contract assigns `R`/`S` for specified tetrahedral elements and
...
nodes for higher-order bonds. Atomic-number spheres are compared before isotope
spheres so isotope differences refine otherwise equivalent ligand paths.
...
Unit tests cover tetrahedral descriptors, double-bond descriptors, unresolved
equivalent ligands, bounded resource failures, and descriptor invalidation after
mutation.
